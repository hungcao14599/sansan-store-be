...
async function main() {
  ...
  const staffPassword = await hash('Staff@123', 10);
  ...
  const samples = [
    {
      ...
      quantity: 48,
      minStock: 12,
      taxCategory: TaxCategory.VAT_10,
    },
    {
      sku: 'SN-MILO-180',
      name: 'Sua Milo 180ml',
    ...
    },
    {
      ...
      quantity: 120,
      minStock: 24,
      taxCategory: TaxCategory.NO_VAT,
    },
    {
      sku: 'SN-DUONG-1KG',
      name: 'Duong cat 1kg',
  ...
  for (const item of samples) {
    ...
    const product = await prisma.product.upsert({
      ...
      update: {
        ...
        taxRate: item.taxCategory === TaxCategory.NO_VAT ? '0' : '10',
        isActive: true,
      },
      create: {
        sku: item.sku,
  ...
  if (!demoOrder) {
    ...
    const hasRevenueLog = demoOrder.revenueLogs.some(
    ...
    const hasCollection = demoOrder.paymentTransactions.some(
      (item) => item.type === PaymentTransactionType.COLLECTION,
    );
